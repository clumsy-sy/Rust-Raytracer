...
use rustRaytracer::{
    camera::Camera,
    materials::Material::{*, self}, geometry::{Hittable, Sphere, HittableList}, vector::{Vec3, Point, Color}, renderer::Renderer,
};
use std::{io, char::REPLACEMENT_CHARACTER};

fn main() -> io::Result<()> {
    let mut Box: Vec<Box<dyn Hittable>> = vec![];

    let ground_material = Material::Lambertian{albedo: Color::new(0.5, 0.5, 0.5).into()};
    Box.push(Box::new(Sphere::new(Point::new(0.0, -1000.0, 0.0), 1000.0, ground_material.clone())));
        ...
        for b in -11..11 {
            ...
            if (center - Point::new(4.0, 0.2, 0.0)).length() > 0.9 {
                if choose_mat < 0.8 {
                    ...
                    let sphere_material = Material::Lambertian{ albedo: albedo.into() };
    ...
    Box.push(Box::new(Sphere::new(Point::new(0.0, 1.0, 0.0), 1.0, material1.clone())));

    let material2 = Material::Lambertian{albedo: Color::new(0.4, 0.2, 0.1).into()};
    Box.push(Box::new(Sphere::new(Point::new(-4.0, 1.0, 0.0), 1.0, material2.clone())));

    let material3 = Material::Metal{albedo: Color::new(0.7, 0.6, 0.5), fuzz: 0.0};
    Box.push(Box::new(Sphere::new(Point::new(4.0, 1.0, 0.0), 1.0, material3.clone())));
